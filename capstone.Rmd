...
```{r load data from cache if exists}
need_load <- TRUE;
...
```{r calculate percentage of pitch types}
pitches_dl_dataset$num_pitches <- sqrt(pitches_dl_dataset$num_pitches);
pitches_dl_dataset$num_AB <- sqrt(pitches_dl_dataset$num_AB);
pitches_dl_dataset$num_AS <- sqrt(pitches_dl_dataset$num_AS);
pitches_dl_dataset$num_CH <- sqrt(pitches_dl_dataset$num_CH);
pitches_dl_dataset$num_CU <- sqrt(pitches_dl_dataset$num_CU);
pitches_dl_dataset$num_EP <- sqrt(pitches_dl_dataset$num_EP);
pitches_dl_dataset$num_FA <- sqrt(pitches_dl_dataset$num_FA);
pitches_dl_dataset$num_FC <- sqrt(pitches_dl_dataset$num_FC);
pitches_dl_dataset$num_FF <- sqrt(pitches_dl_dataset$num_FF);
pitches_dl_dataset$num_FO <- sqrt(pitches_dl_dataset$num_FO);
pitches_dl_dataset$num_FS <- sqrt(pitches_dl_dataset$num_FS);
pitches_dl_dataset$num_FT <- sqrt(pitches_dl_dataset$num_FT);
pitches_dl_dataset$num_IN <- sqrt(pitches_dl_dataset$num_IN);
pitches_dl_dataset$num_KC <- sqrt(pitches_dl_dataset$num_KC);
pitches_dl_dataset$num_KN <- sqrt(pitches_dl_dataset$num_KN);
pitches_dl_dataset$num_PO <- sqrt(pitches_dl_dataset$num_PO);
pitches_dl_dataset$num_SC <- sqrt(pitches_dl_dataset$num_SC);
pitches_dl_dataset$num_SI <- sqrt(pitches_dl_dataset$num_SI);
pitches_dl_dataset$num_UN <- sqrt(pitches_dl_dataset$num_UN);
...
numeric_dataset <- numeric_dataset[3:(ncol(numeric_dataset)-1)];
m <- cor(numeric_dataset);
corrplot(m);
```

```{r PCA}
standardised <- as.data.frame(scale(numeric_dataset));
...
```{r transform}
trans <- preProcess(pitches_dl_dataset, method=c("BoxCox", "center", "scale"))

pitches_dl_dataset_transform <- predict(trans, pitches_dl_dataset);

d <- melt(numeric_dataset_trans);
...
#for (i in 3:ncol(pitches_dl_dataset[,1: ncol(pitches_dl_dataset) - 1 ])){  
for (i in 3:(ncol(pitches_dl_dataset)-2)){  
...
train <- createDataPartition(pitches_dl_dataset$OnDL, p=0.7, list=FALSE);
