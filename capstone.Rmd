...
# 
# for (t in c("AB", "AS", "CH", "CU", "EP", "FA", "FC", "FF", "FO", "FS", "FT", "IN","KC", "KN", "PO", "SC", "SI", "SL", "UN"))
# {
#   expression <-  paste("pitches_dl_dataset$num_", t, sep="");
#   eval_expression <- eval(parse(text=expression)) /pitches_dl_dataset$num_pitches;
#   assign_var = paste("pitches_dl_dataset$trf_num_", t, sep="");
#   assign(assign_var,  eval_expression)
#   
#   expression <-  paste("pitches_dl_dataset$num_", t, sep="");
#   eval_expression <- eval(parse(text=expression))/pitches_dl_predict$num_pitches;
...
```{r warning=FALSE}
pred <- ifelse(predict(mod_1b, testing_lessOutliers, type='response') > threshold,  1, 0)
...
legend(0.3,0.4,       c("Model 1","Model 1b"), lty=c(1,1), lwd=c(2.5,2.5),col=c("red","blue"), pch=1, cex=0.5);
```
...
### Performance
```{r warning=FALSE}
pred <- ifelse(predict(mod_2, testing_lessOutliers, type='response') > threshold,  1, 0);
confusionMatrix(data=pred, reference=testing_lessOutliers$OnDL, positive='1');
```
### ROC curve
```{r warning=FALSE}
...
### Performance
```{r warning=FALSE}
pred <- ifelse(predict(mod_2b, testing_lessOutliers, type='response') > threshold,  1, 0);
confusionMatrix(data=pred, reference=testing_lessOutliers$OnDL, positive='1');
```
### ROC curve
```{r warning=FALSE}
...
### Performance
```{r warning=FALSE}
pred <- ifelse(predict(mod_2c, testing_lessOutliers, type='response') > threshold,  1, 0);
confusionMatrix(data=pred, reference=testing_lessOutliers$OnDL, positive='1');
```
...
### Performance
```{r warning=FALSE}
pred <- ifelse(predict(mod_2d, testing_lessOutliers, type='response') > threshold,  1, 0);
confusionMatrix(data=pred, reference=testing_lessOutliers$OnDL, positive='1');
```
...
### Performance
```{r warning=FALSE}
pred <- ifelse(predict(mod_3, testing_lessOutliers, type='response') > threshold,  1, 0);
confusionMatrix(data=pred, reference=testing_lessOutliers$OnDL, positive='1');
```
...
## Model 3(b) Siginificant Continuous Variables + num pitches
...
### Performance
```{r warning=FALSE}
pred <- ifelse(predict(mod_3b, testing_lessOutliers, type='response') > threshold,  1, 0);
confusionMatrix(data=pred, reference=testing_lessOutliers$OnDL, positive='1');
```
...
legend(0.3,0.4,       c("Model 1b","Model 2","Model 2b","Model 2c","Model 3","Model 3b"), lty=c(1,1), lwd=c(2.5,2.5),col=c("red","blue","green","deeppink","skyblue","limegreen"), pch=1, cex=0.5);
```
...
selected_variables <- c("y","z0", "ay", "trf_num_FA", "trf_num_FC", "trf_num_FF", "trf_num_FT", "trf_num_IN", "trf_num_SI");  
...
Model 2b has the highest accuracry rate.  
```{r warning=FALSE}
...
dbhandle <- odbcDriverConnect('driver={SQL Server};server=localhost;database=Lahman;trusted_connection=true');
...
<!-- # Oversampling -->
<!-- ```{r warning=FALSE} -->
<!-- prop.table(table(training_lessOutliers$OnDL)) -->

<!-- selected_variables <- dependent_var; # c("y", "start_speed", "sz_bot", "vy0", "break_y", "trf_num_pitches", "trf_num_FA"); -->
...
<!-- library('ROSE') -->
<!-- library('rpart') -->
<!-- mod_imb <- glm(formula = formula , family=binomial(logit), data=training_lessOutliers); -->
<!-- pred_imb <- predict(mod_3b, testing_lessOutliers) -->

<!-- accuracy.meas(testing_lessOutliers$OnDL, pred_imb) -->

<!-- roc.curve(testing_lessOutliers$OnDL, pred_imb, plotit = F) -->
<!-- ``` -->

<!-- ```{r warning=FALSE} -->
<!-- data_balanced_over <- ovun.sample(formula = formula, data=training_lessOutliers, method="over", N=4744, seed=1)$data; -->

<!-- #data_balanced_over <- ROSE(formula = formula, data=training_lessOutliers, seed=1)$data; -->


<!-- table(data_balanced_over$OnDL) -->

<!-- mod_bal <- glm(formula = formula , family=binomial(logit), data=data_balanced_over); -->
<!-- summary(mod_bal) -->
<!-- pred_bal <- predict(mod_bal, testing_lessOutliers) -->

<!-- accuracy.meas(testing_lessOutliers$OnDL, pred_bal, threshold = 0.4) -->

<!-- roc.curve(testing_lessOutliers$OnDL, pred_bal, plotit =F) -->
<!-- # pred <- ifelse(predict(pred_bal, testing_lessOutliers, type='response') > threshold,  1, 0); -->
<!-- # confusionMatrix(data=pred, reference=testing_lessOutliers$OnDL, positive='1'); -->

<!-- prob <- predict(mod_bal, testing_lessOutliers, type='response'); -->
<!-- gbal <- roc(OnDL ~ prob, data = testing_lessOutliers); -->
<!-- plot(gbal) -->
<!-- ``` -->
<!-- Random Forest -->
...
<!-- formula_text <- paste("as.factor(", response_var, ")", "~", -->
<!--                       paste(names(training_lessOutliers)[selected_i], collapse="+")); -->
<!-- formula <- as.formula(formula_text); -->


<!-- rf <- randomForest(formula, training_lessOutliers, ntree=500, importance=T) -->
<!-- getTree(rf, 1) -->
<!-- # mod_rf <- train(formula = formula , method="rf", data=data_balanced_over, prox=TRUE); -->
<!-- summary(rf) -->
<!-- pred_bal <- predict(rf, testing_lessOutliers, type="response") -->

<!-- accuracy.meas(testing_lessOutliers$OnDL, pred_bal, threshold = 0.4) -->

<!-- library('party'); -->
<!-- rf_tree <- ctree(formula, data=training_lessOutliers) -->
<!-- plot(rf_tree, type="simple") -->

<!-- roc.curve(testing_lessOutliers$OnDL, pred_bal, plotit =F) -->
<!-- # pred <- ifelse(predict(pred_bal, testing_lessOutliers, type='response') > threshold,  1, 0); -->

<!-- pred <- ifelse(predict(rf, testing_lessOutliers, type='response') > threshold,  1, 0); -->
<!-- confusionMatrix(data=pred, reference=testing_lessOutliers$OnDL, positive='1'); -->
